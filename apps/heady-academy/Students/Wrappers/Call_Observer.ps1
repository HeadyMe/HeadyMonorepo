...
# FILE: apps/heady-academy/Students/Wrappers/Call_Observer.ps1
# LAYER: root
# 
...
try {
    ...
    $observerTool = Join-Path $BASE "Tools\Daemons\Natural_Observer.py"
    if (-not (Test-ToolExists $observerTool)) { throw "Observer daemon missing at $observerTool" }

    Write-Host "[OBSERVER] Self-check passed. Wrapper is ready for use." -ForegroundColor DarkGreen
} catch {
    Write-Host "[OBSERVER] Self-check failed: $($_.Exception.Message)" -ForegroundColor Red
}
# Final validation and cleanup
if ($MyInvocation.InvocationName -eq '.' -or $MyInvocation.Line -match 'Import-Module') {
    # Script is being dot-sourced or imported - skip execution, just validate functions
    $requiredFunctions = @('Show-Help', 'Test-Python', 'Test-ToolExists', 'Start-Observer')
    $missingFunctions = $requiredFunctions | Where-Object { -not (Get-Command $_ -ErrorAction SilentlyContinue) }
    
    if ($missingFunctions.Count -eq 0) {
        Write-Verbose "[OBSERVER] All functions loaded successfully"
    } else {
        Write-Warning "[OBSERVER] Missing functions: $($missingFunctions -join ', ')"
    }
}
