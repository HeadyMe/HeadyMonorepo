...
# FILE: apps/heady-academy/Students/Wrappers/Call_Sentinel.ps1
# LAYER: root
# 
...
if ($env:HEADY_SELFTEST -eq "1") {
    Write-Host "[SENTINEL] Self-test mode" -ForegroundColor Yellow
    if (Test-Python) {
        Write-Host "✓ Python available" -ForegroundColor Green
    }
    $toolPath = "$BASE\Tools\Access_Ledger.py"
    if (Test-Path $toolPath) {
        Write-Host "✓ Tool exists: $toolPath" -ForegroundColor Green
    } else {
        Write-Host "✗ Tool missing: $toolPath" -ForegroundColor Red
    }
    exit 0
}
