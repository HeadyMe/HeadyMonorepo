...
// FILE: apps/manager/_archive/heady-manager.old.js
// LAYER: root
// 
...
const { httpMetricsMiddleware, register: metricsRegister, performHealthCheck } = require('./src/utils/monitoring');
const { cache } = require('./src/utils/cache');
const RealtimeManager = require('./src/utils/realtime');
const fs = require('fs');
...
// Serve Sacred Geometry UIs
app.get('/admin', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'admin-sacred.html'));
});
...
// File operations: create new file
app.post(
  '/api/admin/file/create',
  asyncHandler(async (req, res) => {
    const { root: rootParam, path: relPath, content = '', type = 'file' } = req.body || {};
    if (typeof relPath !== 'string' || !relPath) {
      throw createHttpError(400, 'path is required');
    }
    ...
    if (fs.existsSync(targetPath)) {
      throw createHttpError(409, 'File or directory already exists');
    }
...
// File operations: delete file or directory
app.delete(
  '/api/admin/file',
  asyncHandler(async (req, res) => {
    ...
    if (!fs.existsSync(targetPath)) {
      throw createHttpError(404, 'File or directory not found');
    }
...
// File operations: rename/move file or directory
app.put(
  '/api/admin/file/rename',
  asyncHandler(async (req, res) => {
    const { root: rootParam, path: relPath, newPath: newRelPath } = req.body || {};
    if (typeof relPath !== 'string' || !relPath) {
      throw createHttpError(400, 'path is required');
    }
    if (typeof newRelPath !== 'string' || !newRelPath) {
      throw createHttpError(400, 'newPath is required');
    }

    const root = assertAdminRoot(rootParam);
    const sourcePath = resolveAdminPath(root.path, relPath);
    const targetPath = resolveAdminPath(root.path, newRelPath);

    if (!fs.existsSync(sourcePath)) {
      throw createHttpError(404, 'Source file or directory not found');
    }

    if (fs.existsSync(targetPath)) {
      throw createHttpError(409, 'Target path already exists');
    }
    ...
    res.json({
      ok: true,
      root,
      oldPath: toRelativePath(root.path, sourcePath),
      newPath: toRelativePath(root.path, targetPath),
      renamed: true,
    });
  }),
);
...
const server = app.listen(PORT, () => {
  logger.info(`Heady Manager Server starting...`);
  logger.info(`Running on port ${PORT}`);
  logger.info(`Admin UI: http://localhost:${PORT}/admin`);
  logger.info(`Health: http://localhost:${PORT}/api/health`);
  logger.info(`Metrics: http://localhost:${PORT}/api/metrics`);
  logger.info(`API Key: ${process.env.HEADY_API_KEY ? 'Set' : 'Not set'}`);
  logger.info(`HF Token: ${process.env.HF_TOKEN ? 'Set' : 'Not set'}`);
});
